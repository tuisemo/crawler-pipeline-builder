...
class FakePage:
    ...
    def wait_for_timeout(self, timeout):
        ...
        if self._timeout_callbacks:
            callback = self._timeout_callbacks.pop(0)
            callback()
...
async def test_subflow_step_limit_returns_partial_structured_failure(monkeypatch):
    session = FakeSession()
    monkeypatch.setattr("backend.workflow.executor.page_session_mgr.create", lambda: session)
    monkeypatch.setattr("backend.workflow.executor.INTERNAL_EXECUTION_STEP_BUDGET", 1)
...
@pytest.mark.anyio
async def test_subflow_paginate_confirms_content_change_when_url_and_count_stay_same(monkeypatch):
    session = FakeSession()
    first_item = FakeElement(text="One", children={".name": [FakeElement(text="One")]})
    second_item = FakeElement(text="Two", children={".name": [FakeElement(text="Two")]})

    def move_to_page_two():
        session.page._timeout_callbacks.append(
            lambda: session.page._selectors.__setitem__(".item", [second_item])
        )
        session.page._selectors["a.next"] = []
    ...
    assert response.records == [{"_index": 0, "name": "One"}, {"_index": 0, "name": "Two"}]
    assert response.node_results[3].result["advanced"] is True
    assert response.node_results[3].result["message"] == "Advanced to page 2"


@pytest.mark.anyio
async def test_subflow_paginate_does_not_requeue_when_content_did_not_change(monkeypatch):
    session = FakeSession()
    first_item = FakeElement(text="One", children={".name": [FakeElement(text="One")]})
    ...
    assert response.success is True
    assert [result.node_id for result in response.node_results] == ["open", "list", "extract", "page"]
    assert response.records == [{"_index": 0, "name": "One"}]
    assert response.node_results[-1].result["advanced"] is False
    assert "did not advance" in response.node_results[-1].result["message"]
...
async def test_subflow_emit_record_can_persist_sqlite(monkeypatch):
    ...
    monkeypatch.setattr("backend.workflow.executor.page_session_mgr.create", lambda: session)
    monkeypatch.setattr("backend.runtime.record_sinks.WORKSPACE_ROOT", workspace_root)
...
async def test_test_node_unknown_session_reports_expired(monkeypatch):
    ...
    monkeypatch.setattr("backend.workflow.executor.page_session_mgr.get", lambda session_id: None)
    request = TestNodeRequest.model_validate({
