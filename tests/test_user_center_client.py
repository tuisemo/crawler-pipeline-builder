...
def test_build_authorize_url_delegates_to_authlib(monkeypatch):
    ...
    monkeypatch.setattr("backend.auth.user_center_client.create_oauth_client", lambda redirect_uri: fake_client)
    monkeypatch.setattr("backend.auth.user_center_client.get_authorize_url", lambda: "https://uc.example.com/oauth/authorize")
...
async def test_exchange_code_for_token_returns_access_token(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.auth.user_center_client.create_oauth_client",
        lambda redirect_uri: fake_client,
    )
    monkeypatch.setattr(
        "backend.auth.user_center_client.get_access_token_url",
...
async def test_exchange_code_for_user_gets_openid_from_user_get(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.auth.user_center_client.exchange_code_for_token",
    ...
    with patch("backend.auth.user_center_client.httpx.AsyncClient", mock_cls):
