...
from backend.workflow.output_defaults import (
    DEFAULT_BATCH_SIZE,
...
def collect_list_snapshot(page, item_selector: str) -> dict[str, Any]:
    items = page.query_selector_all(item_selector) if item_selector else []
    return {{
        "url": getattr(page, "url", None),
        "item_count": len(items) if item_selector else None,
        "item_signatures": [item_signature(item) for item in items[:3]],
    }}


def pagination_state_changed(before: dict[str, Any], after: dict[str, Any]) -> bool:
...
def wait_for_list_update(page, item_selector: str, before_snapshot: dict[str, Any]) -> bool:
    try:
        page.wait_for_load_state("networkidle", timeout=remaining_timeout_ms(5000, reserve_seconds=3))
    except Exception:
        pass
    try:
    ...
    after_snapshot = collect_list_snapshot(page, item_selector)
    if pagination_state_changed(before_snapshot, after_snapshot):
        return True

    deadline = time.monotonic() + (remaining_timeout_ms(6000, reserve_seconds=2) / 1000)
