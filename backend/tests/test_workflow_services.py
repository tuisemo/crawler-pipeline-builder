...
from workflow.detail_batch_prompting import (
...
def test_validate_graph_raises_on_duplicate_edge_ids():
    ...
    from workflow.schemas import WorkflowEdge
    request = ValidateWorkflowRequest(
        graph=WorkflowGraph(
            nodes=[
                WorkflowNode(id="n1", type="open_page", data=NodeData(url="http://a.com")),
...
def test_generate_crawler_revises_script_when_review_requests_changes(monkeypatch):
    ...
    monkeypatch.setattr("workflow.generation_pipeline.get_default_client", lambda: fake_client)
...
def test_generate_crawler_records_sandbox_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("workflow.generation_pipeline.get_default_client", lambda: FakeClient())
    monkeypatch.setattr("workflow.script_sandbox.SANDBOX_ROOT", tmp_path / "script-sandbox")
...
def test_generate_crawler_rejects_legacy_field_aliases(monkeypatch):
    monkeypatch.setattr(
        "workflow.generation_pipeline.get_default_client",
...
def test_generate_detail_batch_runner_supports_optional_llm_enhancement(monkeypatch):
    ...
    monkeypatch.setattr("workflow.detail_batch_generation_pipeline.get_default_client", lambda: FakeClient())
...
def test_save_script_writes_inside_workspace(monkeypatch):
    ...
    monkeypatch.setattr("workflow.script_artifacts.WORKSPACE_ROOT", workspace_root)
...
def test_service_module_does_not_use_json_response():
    ...
    import workflow.services as services_module
    import inspect
