...
from core.settings import CrawlerWorkflowSettings
...
def test_frontend_proxy_default_matches_backend_default_port():
    vite_config = (WORKSPACE_ROOT / "frontend" / "vite.config.ts").read_text(encoding="utf-8")
    frontend_env = (WORKSPACE_ROOT / "frontend" / ".env.development").read_text(encoding="utf-8")
...
def test_declared_python_modules_exist():
    ...
    modules = pyproject.get("tool", {}).get("setuptools", {}).get("py-modules", [])

    for module_name in modules:
        assert (SERVER_ROOT / f"{module_name}.py").exists(), f"{module_name}.py is declared but missing"
