"""Workflow prompt assembly helpers."""
...
def resolve_output_mode(plan_dict: dict) -> str:
...
def build_output_strategy_prompt(plan_dict: dict) -> str:
    output = plan_dict.get("output", {})
    if not isinstance(output, dict):
        output = {}

    output_mode = resolve_output_mode(plan_dict)
...
def build_model_guardrails_prompt(plan_dict: dict) -> str:
    ...
    return (
        ...
        "- After a pagination click or scroll action, confirm that the list content actually updated before extracting the next page; do not rely only on fixed sleeps or click success.\n"
...
def build_quality_gate_prompt(plan_dict: dict) -> str:
    pagination = plan_dict.get("pagination", {})
    if not isinstance(pagination, dict):
        pagination = {}
    output = plan_dict.get("output", {})
    ...
    return (
        ...
        "- Check 7: after pagination, the script confirms the list content updated before extracting the next page.\n"
        "- Check 8: avoid unnecessary complexity; prefer the simplest deterministic code path that satisfies the plan.\n\n"
    ).format(
...
def build_review_prompt(plan_dict: dict, generated_script: str) -> str:
    return (
        ...
        "```\n\n"
        f"{build_output_strategy_prompt(plan_dict)}"
        "## Draft Script\n"
        "```python\n"
        f"{generated_script}\n"
        "```\n\n"
