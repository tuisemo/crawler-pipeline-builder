...
import { extractEffectivePrompt, normalizeMultilineText, summarizeText } from '../prompt-workspace/promptDrafts'
import { postWorkflowAction } from '../../services/workflowApi'
import { getErrorMessage } from '../workflow/workflowState'
  ...
  if (focusMode && view === 'script' && normalizedScript) {
    return (
      <div className="result-details-root result-details-focus">
        ...
        {!isDetailBatchRunnerScript ? (
          <Collapse
            size="small"
            className="script-focus-collapse"
            items={[
              {
                key: 'detail-batch-runner-focus',
                label: (
                  <Space size={8} wrap>
                    <Typography.Text type="secondary">详情批处理脚本</Typography.Text>
                    <StatTags value="可选第二阶段" accent="orange" />
                    {detailBatchResultPayload?.script ? <StatTags value="已生成" accent="green" /> : <StatTags value="未生成" />}
                  </Space>
                ),
  ...
  return (
    <div className="result-details-root">
      {shouldRenderStructuredBlocks ? (
        isSpecificView ? (
          <div className="result-specific-layout">
