...
class ReadabilityDetector(BaseDetector):
    ...
    def detect(self, html_content: str, url: str | None = None) -> DetectorResult:
        if not html_content:
            return DetectorResult(content_area="/html/body", confidence=0.0, text_length=0, strategy=DetectorStrategy.READABILITY)
        try:
        ...
        if not candidates:
            return DetectorResult(content_area="/html/body", confidence=0.1, text_length=0, strategy=DetectorStrategy.READABILITY)
        ...
        try:
            ...
            content_area = doc.getpath(best[1])
        except Exception:
            content_area = best[0]
