...
class HeuristicDetector(BaseDetector):
    ...
    def detect(self, html_content: str, url: str | None = None) -> DetectorResult:
        fallback = DetectorResult(content_area="/html/body", confidence=0.0, text_length=0, strategy=DetectorStrategy.HEURISTIC)
        if not html_content:
            return fallback
        try:
        ...
        if best_result is None or best_score < 30:
            ...
            if dynamic and dynamic[2] > best_score:
                best_result = (dynamic[1], dynamic[2])

        if best_result is None:
            return fallback

        element, raw_score = best_result
        ...
        try:
            ...
            content_area = doc.getpath(element)
        except Exception:
            content_area = element.tag
