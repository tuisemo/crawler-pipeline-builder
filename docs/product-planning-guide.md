...
> 状态说明（2026-05-08 之后）：
> 本文中关于 `test-node` / `test-subflow`、后端浏览器会话和 relay 的描述，属于历史产品能力记录。
> 当前产品主路径已经切换为“工作台前端直连本地扩展取证，后端仅保留纯计算与脚本生成”。
...
它不是最终的调度平台，也不是单纯的脚本仓库，而是把“目标站点理解、采集流程设计、页面取证、脚本生成、详情采集编排生成”集中在一个可视化端侧应用里，承担从采集意图到可执行脚本产物的转换职责。
...
sequenceDiagram
    ...
    P->>W: 选择目标站点并配置节点
    W->>B: validate / compile-plan
    B-->>W: 校验结果、编排结果
    W->>W: 通过本地扩展执行选择器测试与页面取证
    P->>W: 触发字段/分页/选择器辅助
    W->>B: assist 请求
    B->>L: 结构化推断
    L-->>B: JSON 结果
...
| 页面取证样本 | 扩展直连的选择器测试 / HTML 提取 / 分页分析 | 验证字段、分页与选择器逻辑 |
